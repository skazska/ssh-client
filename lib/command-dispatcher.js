...
function fileTransfer (command, client, text) {
    return new Promise(resolve => {
        ...
        if (pathFrom) {
            try {
                transferFile.transferFile(command, client, pathFrom, pathTo).then(
                    (result) => {
...
class CommandDispatcher {
    ...
    dispatchCommand (cmd, text) {
        return commands[cmd] && commands[cmd](this.client, text);
    }
...
module.exports = {
    get: (client) => { return new CommandDispatcher(client) },
